...
func (transMgr *transactionMgr_t) NewOrder(order Button_t) {
	if order.ButtonType == CMD {
		if DEBUG_TRNSMGR {
			fmt.Printf("transMgr: did not send new order (%+v) since ButtonType == CMD\n", order)
		}
		return
	}
...
func (transMgr *transactionMgr_t) RequestOrder(order Button_t, cost int) {
	...
	*/
	...
	if transMgr.nElevatorsOnline() <= 1 {
		...
		if DEBUG_TRNSMGR {
			fmt.Printf("transMgr: Request UP/DOWN Order on %+v with cost %d and %d elevs online\n", order, cost, transMgr.nElevatorsOnline())
		}
		transMgr.SendCost(order, cost)
	}
...
func (transMgr *transactionMgr_t) SendSync(rawQue []byte) {
	if DEBUG_TRNSMGR {
		fmt.Printf("transMgr: SendSync got que from elevMgr, sending que on network\n")
	}
